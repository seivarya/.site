...
about arena allocators much, but on an abstract level instead of moving the break of the whole program, we allocate a very huge chunk of memory at once using `malloc` or `mmap` and then treat the block itself as a *"sandbox"*...